...
| Claude agent stream (SDK control protocol; each tool call as a typed item) | `agent/claude.go`, `agent/claude_tools.go`, `sandbox/runtime.go` `Stream` | | | `agent/claude_test.go` | [warden-resident-claude-plan](warden-resident-claude-plan.md), [claude-parity](claude-parity.md) (feature inventory and priority order) |
| Tool cards: each agent tool call as a typed transcript card — a command with its output folded, a file change as a diff (Claude's Edit/Write with the CLI's numbered hunks), a read with its path, a search with its hits, a fetch, an MCP call, a subagent — on the web and the TUI | `conversation/model.go` (`Tool`), `conversation/conversation.go` (`Upsert`, `toolStatus`), `agent/claude_tools.go` (`claudeToolItem`) | `GET state` / `events` → `entries[].tool` | `ToolCard.tsx` (+ `tools.ts`, `tools.test.ts`), `EntryView.tsx`, `DiffView.tsx`; TUI `tui/render.go` (`renderTool`) | `agent/claude_test.go`, `conversation/conversation_test.go`, `tui/tui_test.go`, `web/src/tools.test.ts` | [claude-parity](claude-parity.md) |
...
| Terminal client `warden chat` (TUI), `chat list|new|send --wait|approve`: `/` menu and `@path` completion, `/attach` (uploads sent with the next message), `/export [md|json] [all] [FILE]` (the web's markdown and JSON), `/rename` `/archive` `/restore` `/delete`, Claude Code's keys (Ctrl+C/D, Esc interrupts, Ctrl+O, Ctrl+L, Ctrl+A/E/U/K/W, Ctrl+R and Up/Down over a per-chat prompt history under `<state>/tui/history/`), status line with the run's elapsed time, pending approvals and the turn's tokens/cost | `tui/` | same HTTP API (`chats/{id}/paths`, `chats/{id}/attachments`, `chats/{id}/edit`, `environments/{id}/delete`) | `tui/app.go`, `tui/complete.go`, `tui/export.go`, `tui/history.go`, `tui/status.go`, `tui/editor.go`, `tui/follow.go` | `tui/tui_test.go` | [claude-parity](claude-parity.md) (item 6) |
