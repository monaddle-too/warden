...
| Slash commands of the agent's session (Claude Code's built-ins, the workspace's commands and skills) in the composer's `/` menu; `/name …` sent as text for the CLI to expand; `/compact` leaves a system line with the token counts | `agent/claude.go` (`system/init` → `thread/started` `commands`/`model`/`permissionMode`/`outputStyle`, `system/compact_boundary` → `thread/compacted`), `chats/store.go` (`Chat.Commands`, `Chat.Session`, `sessionStarted`), `chats/engine.go` (`compactionNote`) | `GET state` / `events` → `chat.commands`, `chat.session` | `composer.ts` (`commandItems`, `agentCommandNamed`, `AGENT_HINTS`), `Conversation.tsx`, `Suggest.tsx` (groups); TUI not yet | `agent/claude_test.go`, `chats/engine_test.go`, `web/src/composer.test.ts` | [claude-parity](claude-parity.md) items 5 and 7 |
