...
- 2026-09-17: step 15, final review — build, web tests (99) and Go `chats`, `sandbox`, `agent`, `conversation` tests green (`go vet` clean); the branch diff against 21e7800 (76 files) read end to end. Fixed: `touchesEnd` misread an unterminated fence or `$$` inside a blockquote as closed whenever a chunk ended on a newline (micromark ends a quoted block before the line ending), so `openFrom` in `streaming.ts` now counts an open block from the end of its last line when the text ends inside a fence or `$$` block (one scan, `openBlocks`, replaces `fenceOpen`), with tests for quoted, listed and top-level blocks; an open ```` ```math ```` fence is held as plain code by `holdOpenMath` like an open `$$`; held display math is rebuilt from its content so a quoted formula does not show `> ` prefixes. Consolidated: `AttachmentFile` used its own copy of `saveFile`, `plural` lived in both `ChatShell.tsx` and `ExportDialog.tsx` (now in `export.ts`), the new `credentials()` header helper in `api.ts` is used by every fetch the branch touched. Smaller: `imageLoader` forgets a failed fetch only if its own promise is still the cached one; `searchChats` no longer caches the folded text of a streaming entry (each chunk was a new cache row); `/model x` typed in full while the agent runs is refused locally with the service's wording; with every suggestion row disabled the arrow keys move the caret again; the collapsed code block's `max-height` now matches 40 lines of the `<code>` inside (0.9 em). Main chunk 513.7 → 513.8 kB; against the 454.6 kB before step 1 the branch adds 59.2 kB (+13 %) to the main chunk, with the highlighter (164 kB), KaTeX (259 + 11 kB JS, 29 kB CSS) and Mermaid (largest chunk 662 kB) as lazy chunks loaded only by a message that needs them.
- 2026-09-17: live test on the local install (`scripts/deploy-local.sh`, dev build `74c7ce2`, real sandboxes, both providers). Claude chat: dropped PNG + text file arrived as an image block and as `.warden/attachments/<id>.txt` (the agent described the image and printed the file), agent-made `images/plot.png` rendered inline through `image-file` with the lightbox, Mermaid flowchart with Source toggle, highlighted Go/Python with Wrap/Copy, an 83-line block folded on a fresh mount and stayed open while streamed, ```` ```diff ```` fence and display/inline KaTeX, table, "24s · 138k tokens · $0.07" turn line, "NEW MESSAGES" divider and "2 new messages" pill after switching chats mid-turn, ⌘K search across chats with snippets, in-chat find "1 of 2", Export dialog, Edit prefilling the composer, Retry sending a new entry, `/` menu, `@im` → `images/` → Tab → `images/plot.png` against the live sandbox. Codex chat: dropped PNG described correctly (`localImage` works), `![…](https://…)` stayed alt text with no request, a `https://github.com/…` label on an example.com link showed "→ example.com", an `<img>` in a Mermaid label was stripped, `apply_patch` step rendered as a diff with line numbers, tokens shown without a cost. Light scheme re-themed Mermaid and the highlighter. Found and fixed: a message sent while scrolled up headed the unread stretch on the next visit (the seen mark only advances while following); `unreadStart` now skips the reader's own messages and `send()` follows before the request. Not exercised here: ⌘/Ctrl+Enter (the test browser cannot emulate it), the Export download itself, and copy-to-clipboard.
