...
func TestRunnerSelectsDriverByRuntimeKind(t *testing.T) {
	...
	driver, err := runtimeDriver(s, sizeLimits(s), "")
	if err != nil || driver == nil {
		t.Fatal(err)
	}
	unknown := s
	unknown.cfg.Runtime.Kind = "firecracker"
	if _, err = runtimeDriver(unknown, sizeLimits(unknown), ""); err == nil {
	...
	if _, err = runtimeDriver(s, sizeLimits(s), filepath.Join(dir, "missing")); err == nil || !strings.Contains(err.Error(), "kubernetes API access") {
		t.Fatalf("missing kubeconfig: %v", err)
	}
	driver, err = runtimeDriver(s, sizeLimits(s), kubeconfig)
	if err != nil || driver == nil {
		t.Fatal(err)
	}
	w := &sandbox.Worker{}
	...
	opts := kubernetesOptions(s.cfg.Kubernetes, sizeLimits(s).Default)
