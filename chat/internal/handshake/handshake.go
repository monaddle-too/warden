...
// sockets before it serves anything, and `warden start` reads them from
// each binary's --version output before launching. A different protocol is
...
// Runner asks a warden-runner worker socket for its identity with the
// worker protocol's "health" operation, which every runner build answers
// with its revision and protocol number.
func Runner(ctx context.Context, socket string) (Peer, error) {
	var response struct {
	...
	if err := exchange(ctx, socket, request, &response); err != nil {
		return Peer{}, err
	}
...
// Policy asks a warden-policy control socket for its identity with the
// control protocol's "version" operation. A policy service that rejects the
// operation returns ErrNoHandshake.
func Policy(ctx context.Context, socket string) (Peer, error) {
	var response struct {
		OK       bool   `json:"ok"`
...
// dialError marks a failure to reach the socket at all, which Verify retries
// while the peer may still be starting.
type dialError struct{ error }

func (e dialError) Unwrap() error { return e.error }

func exchange(ctx context.Context, socket string, request any, response any) error {
...
type Options struct {
	// Wait is how long a peer whose socket is not yet accepting connections
	// is retried before it is reported unreachable (it may still be starting).
	Wait time.Duration
...
// Verify checks the runner and the policy service (either socket may be ""
...
func Verify(ctx context.Context, self Peer, runnerSocket, policySocket string, o Options) ([]Peer, error) {
	warn := o.Warn
	if warn == nil {
		warn = func(string) {}
	}
	...
	checks := []struct {
		name, socket string
		ask          func(context.Context, string) (Peer, error)
	}{
		{"warden-runner", runnerSocket, Runner},
		{"warden-policy", policySocket, Policy},
	}
	for _, c := range checks {
		if c.socket == "" {
			continue
		}
		peer, err := retry(ctx, o.Wait, func() (Peer, error) { return c.ask(ctx, c.socket) })
		switch {
		case errors.Is(err, ErrNoHandshake):
			warn(fmt.Sprintf("%s at %s predates the version handshake (revision unknown); update it to %s", c.name, c.socket, self.Revision))
			continue
		case err != nil:
			warn(fmt.Sprintf("could not verify the version of %s at %s: %v", c.name, c.socket, err))
			continue
		}
