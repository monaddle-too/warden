...
func sharingTools() []any {
	return append(imageTools(), []any{
		...
		map[string]any{"type": "function", "name": "list_shared_repositories", "description": "List the GitHub repositories shared with this conversation, with clone and API URLs. Persistent read-only access lasts until the user removes a repository. Use HTTPS Git or GitHub REST through the sandbox proxy; never request credentials. Check this tool when you need repository access.", "inputSchema": map[string]any{"type": "object", "properties": map[string]any{}, "additionalProperties": false}},
