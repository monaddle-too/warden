...
func sharingTools() []any {
	return append(imageTools(), []any{
		...
		map[string]any{"type": "function", "name": "request_google_docs_access", "description": "Ask the user to select Google documents or spreadsheets and an access duration for read or write permission (default read). Write permits Docs batchUpdate text insertion/deletion and styling, and Sheets value and structure updates, on the selected IDs only. Waits for their decision; returns the selected IDs with their API URLs (docs.googleapis.com for documents, sheets.googleapis.com for spreadsheets). Read them with HTTPS requests through the sandbox proxy. Never request Google credentials.", "inputSchema": map[string]any{"type": "object", "properties": map[string]any{"reason": map[string]any{"type": "string", "maxLength": 2000}, "access": map[string]any{"type": "string", "enum": []string{"read", "write"}}}, "required": []string{"reason"}, "additionalProperties": false}},
