...
func TestPermissionCardsAndAnswers(t *testing.T) {
	...
	status := plain(RenderStatus(c, nil, true, 80))
	if !strings.Contains(status, "claude · default · ask") {
		t.Fatalf("status without the mode: %q", status)
	}
	if plain(RenderStatus(sampleChat(), nil, true, 80)) != plain(RenderStatus(sampleChat(), nil, true, 80)) || strings.Contains(plain(RenderStatus(sampleChat(), nil, true, 80)), "auto") {
...
func TestShiftTabCyclesMode(t *testing.T) {
	...
	if !strings.Contains(plain(RenderStatus(app.state.Chats[0], nil, true, 80)), "· plan") {
