...
func TestPodAndClaimSpecGolden(t *testing.T) {
	...
	golden(t, "pod.golden.json", PodSpec(o, "wc-0123456789abcdef01234567", "sandbox-one", "gen-1", false, WorkspaceFresh))
	golden(t, "pod-spare.golden.json", PodSpec(testOptions(), "wc-spare-0123456789abcdef", "", "", true, WorkspaceFresh))
...
func TestPodSpecHardening(t *testing.T) {
	pod := PodSpec(testOptions(), "wc-abc", "s", "g", false, WorkspaceFresh)
	spec := pod.Spec
	...
	spare := PodSpec(testOptions(), "wc-spare-1", "", "", true, WorkspaceFresh)
