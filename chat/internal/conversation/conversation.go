...
func (c *Conversation) Upsert(item map[string]any, turn string, completed bool) {
	...
	case "toolCall":
		...
		e.Tool = &Tool{Kind: agent.String(item["kind"]), Name: agent.String(item["tool"]), Status: toolStatus(item), Paths: paths, Query: agent.String(item["query"]), Input: agent.Map(item["input"]), Background: item["background"] == true, Read: ReadFrom(agent.Map(item["read"]))}
		if e.Tool.Kind == "" {
			e.Tool.Kind = "other"
		}
		if e.Text == "" {
