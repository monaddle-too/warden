...
import { authorLabel, localTime, providerName } from "../export";
import {
  recentChats,
  searchChats,
  snippet,
...
type Row =
  ...
  | { kind: "entry"; chat: Chat; entry: Entry; field: string; match: Match };
...
/* Where an entry's matched text came from, for the row's label. */
              ...
              <>
                ...
                <span className="search-main">
                  <small>
                    {row.chat.title}
                    {" · "}
                    {fieldLabel(row.entry, row.chat.provider, row.field)}
                    {" · "}
