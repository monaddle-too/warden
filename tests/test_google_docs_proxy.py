...
class GoogleDocsProxyTests(unittest.IsolatedAsyncioTestCase):
    setUp = test_proxy.ProxyTests.setUp
    tearDown = test_proxy.ProxyTests.tearDown
    flow = test_proxy.ProxyTests.flow

    async def test_google_docs_uses_protected_route_on_non_github_network(self):
        asyncio.get_event_loop().getaddrinfo.return_value = [(socket.AF_INET,socket.SOCK_STREAM,6,'',('104.18.10.10',443))]
        flow = self.flow(url='https://docs.googleapis.com/v1/documents/TestFile',sni='docs.googleapis.com',method='GET',body=b'',
                         headers={'Authorization':'Bearer guest','X-Goog-Api-Key':'guest-pat','Cookie':'session=guest'})
        flow.request.http_version='HTTP/2.0'
        flow.request.authority='docs.googleapis.com'
        actions = []
        async def dispatch(message):
            actions.append(message)
            if message['action'] == 'authorize':
                self.assertEqual(message['request']['headers'],[])
                return {'allow':True,'authorization':'Bearer google_docs_test_secret','decision_id':'d','request_id':'r','remaining_seconds':60}
            return {'active':True,'recorded':True}
        with patch.object(self.module,'call',dispatch):
            await self.guard.request(flow)
        self.assertIsNone(flow.response)
        self.assertEqual(flow.request.headers['Authorization'],'Bearer google_docs_test_secret')
        ...
        self.assertEqual(flow.server_conn.sni,'docs.googleapis.com')
        self.assertEqual(flow.request.authority,'docs.googleapis.com')
        self.assertNotIn('Host',flow.request.headers)
    ...
    async def test_unsupported_google_docs_channels_cannot_use_public_egress(self):
        asyncio.get_event_loop().getaddrinfo.return_value = [(socket.AF_INET,socket.SOCK_STREAM,6,'',('104.18.10.10',443))]
        for host in ('docs.google.com','drive.google.com','www.googleapis.com','oauth2.googleapis.com'):
